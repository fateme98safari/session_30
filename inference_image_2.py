    ...
    for i in range(h*2):
        for j in range(w*2):
            if result_big[i][j][0] == 0 and result_big[i][j][1] == 0 and result_big[i][j][2] == 0:
                result_big[i][j] = image[int(y-h//2)+i, int(x-w//2)+j]

    image[int(y-h//2):int(y-h//2)+h*2, int(x-w//2):int(x-w//2)+w*2]=result_big
    return image
    # cv2.imshow("result1",result_big)
...
cv2.waitKey()
cv2.imwrite("OpenVtuber/output/result8.jpg",image)
